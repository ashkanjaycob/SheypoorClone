  ...
  const navItems = [
    {
      ...
      icon: (active) =>
        active ? (
          <svg className="w-6 h-6 text-main" viewBox="0 0 24 24" fill="currentColor">
            <path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z" />
          </svg>
        ) : (
          <svg className="w-6 h-6 text-dark-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.8}>
    ...
    },
    {
      ...
      icon: () => (
        <svg className="w-6 h-6 text-dark-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
        ...
        {navItems.map((item, index) => {
          if (item.isAction) {
            return (
              ...
              >
                {item.icon(false)}
                <span className="text-[11px] mt-1 font-medium text-dark-3">
                  {item.label}
                </span>
              </button>
            );
